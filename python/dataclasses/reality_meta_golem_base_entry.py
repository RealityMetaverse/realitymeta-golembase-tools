...
@dataclass
class RealityMetaGolemBaseEntry:
    """
    ...
    # DEV: Golem Base max entry size is 140KB
    ...
    # DEV: for checking if the entry is already stored in the database
    _sys_entry_checksum: Optional[str] = GOLEM_BASE_NULL_VALUE
    ...
    def create_checksums(self) -> tuple[str, str]:
        """Create file and entry checksums."""
        entry_checksum_ignored_fields_set = set(self.ENTRY_CHECKSUM_IGNORED_FIELDS)
        file_checksum_ignored_fields_set = (
            set(self.FILE_CHECKSUM_IGNORED_FIELDS) | entry_checksum_ignored_fields_set
        )
        ...
        if isinstance(self.additional_fields, dict) and len(self.additional_fields) > 0:
            entry_checksum_data = self.additional_fields.copy()
            file_checksum_data = entry_checksum_data.copy()

        for field in fields(self):
            if field.name.startswith(metadata_prefixes):
                field_value = self.__dict__.get(field.name)

                if field.name not in entry_checksum_ignored_fields_set:
                    entry_checksum_data[field.name] = field_value
                if field.name not in file_checksum_ignored_fields_set:
                    file_checksum_data[field.name] = field_value

        entry_checksum = generate_content_hash(entry_checksum_data)
        file_checksum = generate_content_hash(file_checksum_data)

        return entry_checksum, file_checksum
    ...
    def __post_init__(self):
        ...
        self._sys_entry_checksum, self._sys_file_checksum = self.create_checksums()
    ...
    def recreate_file(
        ...
        """
        Recreate a file from this RealityMetaGolemBaseEntry instance.
        """
